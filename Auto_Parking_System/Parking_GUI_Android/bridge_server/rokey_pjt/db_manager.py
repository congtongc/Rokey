...
class DBManager:
    ...
    def __init__(self):
        ...
        print(f"[DEBUG] InfluxDB 연결 정보: host={host}, database={database}, org={org}")
        logger.info(f"[DEBUG] InfluxDB 연결 정보: host={host}, database={database}, org={org}")
    ...
    def get_parking_statistics(self, parked_df=None):
        ...
        try:
            print("[DEBUG] get_parking_statistics 함수 호출됨")
            logger.info("[DEBUG] get_parking_statistics 함수 호출됨")
            
            if parked_df is None:
                print("[DEBUG] parked_df가 None이므로 fetch_current_parked_vehicles 호출")
                logger.info("[DEBUG] parked_df가 None이므로 fetch_current_parked_vehicles 호출")
                parked_df = self.fetch_current_parked_vehicles()
            
            print(f"[DEBUG] parked_df 타입: {type(parked_df)}, 비어있음: {parked_df.empty if hasattr(parked_df, 'empty') else 'N/A'}")
            logger.info(f"[DEBUG] parked_df 타입: {type(parked_df)}, 비어있음: {parked_df.empty if hasattr(parked_df, 'empty') else 'N/A'}")
            
            print("[DEBUG] parked_df 내용:")
            print(parked_df)
            logger.info(f"[DEBUG] parked_df 내용:\n{parked_df}")
            
            if parked_df is not None and not parked_df.empty:
                print("[DEBUG] parked_df가 비어있지 않음, 차량 타입별 카운트 계산")
                logger.info("[DEBUG] parked_df가 비어있지 않음, 차량 타입별 카운트 계산")
                
                for car_type in ["normal", "ev", "disabled"]:
                    count = len(parked_df[parked_df['car_type'] == car_type])
                    print(f"[DEBUG] car_type={car_type}, count={count}")
                    logger.info(f"[DEBUG] car_type={car_type}, count={count}")
                    statistics["occupied"][car_type] = count
                    statistics["available"][car_type] = max(0, statistics["total"][car_type] - count)
            else:
                print("[DEBUG] parked_df가 비어있거나 None임, 기본값 사용")
                logger.info("[DEBUG] parked_df가 비어있거나 None임, 기본값 사용")
                # 기본값 설정 (전체=2, 점유=0, 잔여=2)
                for car_type in ["normal", "ev", "disabled"]:
                    statistics["occupied"][car_type] = 0
                    statistics["available"][car_type] = statistics["total"][car_type]
            
            print(f"[DEBUG] 최종 statistics: {statistics}")
            logger.info(f"[DEBUG] 최종 statistics: {statistics}")
            return statistics
        except Exception as e:
            logger.error(f"주차장 통계 조회 중 오류 발생: {e}")
            print(f"[ERROR] 주차장 통계 조회 중 오류 발생: {e}")
            import traceback
            traceback.print_exc()
            # 오류 발생 시에도 기본값 반환
            return statistics
